  ...
  <div class="bg-white">
    ...
    <main class="pt-10 sm:pt-16">
      ...
      <div class="mx-auto mt-6 max-w-2xl sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:gap-8 lg:px-8">
        <img :src="product.images[0].src" :alt="product.images[0].alt" class="row-span-2 aspect-[3/4] size-full rounded-lg object-cover max-lg:hidden" />
        <img :src="product.images[1].src" :alt="product.images[1].alt" class="col-start-2 aspect-[3/2] size-full rounded-lg object-cover max-lg:hidden" />
        <img :src="product.images[2].src" :alt="product.images[2].alt" class="col-start-2 row-start-2 aspect-[3/2] size-full rounded-lg object-cover max-lg:hidden" />
        <img :src="product.images[3].src" :alt="product.images[3].alt" class="row-span-2 aspect-[4/5] size-full object-cover sm:rounded-lg lg:aspect-[3/4]" />
      </div>

      <!-- Product info -->
      <div class="mx-auto max-w-2xl px-4 pt-10 sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:grid-rows-[auto_auto_1fr] lg:gap-x-8 lg:px-8 lg:pt-16">
              ...
              <fieldset aria-label="Choose a color" class="mt-4">
                <div class="flex items-center gap-x-3">
                  <div v-for="color in product.colors" :key="color.id" class="flex rounded-full outline outline-1 -outline-offset-1 outline-black/10">
                    <input :aria-label="color.name" type="radio" name="color" :value="color.id" :checked="color === product.colors[0]" :class="[color.classes, 'size-8 appearance-none rounded-full forced-color-adjust-none checked:outline checked:outline-2 checked:outline-offset-2 focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-offset-[3px]']" />
                  </div>
                </div>
              </fieldset>
            </div>
            ...
            <div class="mt-10">
              ...
              <fieldset aria-label="Choose a size" class="mt-4">
                <div class="grid grid-cols-4 gap-3">
                  <label v-for="size in product.sizes" :key="size.id" :aria-label="size.name" class="group relative flex items-center justify-center rounded-md border border-gray-300 bg-white p-3 has-[:checked]:border-indigo-600 has-[:disabled]:border-gray-400 has-[:checked]:bg-indigo-600 has-[:disabled]:bg-gray-200 has-[:disabled]:opacity-25 has-[:focus-visible]:outline has-[:focus-visible]:outline-2 has-[:focus-visible]:outline-offset-2 has-[:focus-visible]:outline-indigo-600">
                    <input type="radio" name="size" :value="size.id" :checked="size === product.sizes[2]" :disabled="!size.inStock" class="absolute inset-0 appearance-none focus:outline focus:outline-0 disabled:cursor-not-allowed" />
                ...
                <div class="order-1 flex items-center sm:flex-col sm:items-start">
                  ...
                  <div class="ml-4 sm:ml-0 sm:mt-4">
                    ...
                    <div class="mt-2 flex items-center">
                      <StarIcon v-for="rating in [0, 1, 2, 3, 4]" :key="rating" :class="[review.rating > rating ? 'text-gray-900' : 'text-gray-200', 'size-5 shrink-0']" aria-hidden="true" />
                    </div>
                  </div>
                </div>
              </div>
