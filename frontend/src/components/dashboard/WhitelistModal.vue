...
<template>
  <UiModal :open="open" @close="emit('close')">
    ...
    <div class="modal-toolbar">
      <UiButton variant="primary" type="button" @click="emit('toggleForm')">
        {{ showForm ? copy.dashboard.hideForm : copy.dashboard.newWhitelist }}
      </UiButton>
    </div>
    ...
    <div v-if="showForm" class="editor-box">
      ...
      />
      <UiButton variant="primary" type="button" :disabled="loading" @click="emit('create')">
    ...
    <div class="whitelist-list">
      <article v-for="wl in lists" :key="wl.id" class="whitelist-card">
        <div v-if="editingId === wl.id" class="editor-box editor-inline">
          <input
            v-model="editName"
            type="text"
            class="field"
        ...
        <div v-else>
          <header class="whitelist-head">
            <h4>{{ wl.name }}</h4>
            <div class="inline-actions">
              <UiButton variant="icon" type="button" @click.stop="emit('edit', wl)">
                {{ copy.dashboard.edit }}
              </UiButton>
              <UiButton variant="iconDanger" type="button" @click.stop="emit('delete', wl.id)">
